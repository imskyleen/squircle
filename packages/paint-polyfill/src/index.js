 ...
 *
 * Modifications by Skyleen - 2025
 * Edited to add support for @layer, animation tracking, and some other features
 */
...
let paintWorklet;
const LAYER_RULE = CSSRule.LAYER_RULE || 12;

let CSS = window.CSS;
if (!CSS) window.CSS = CSS = {};

if (!CSS.supports)
  CSS.supports = function s(property, value) {
...
'Hz Q ch cm deg dpcm dpi ddpx em ex fr grad in kHz mm ms number pc percent pt px rad rem s turn vh vmax vmin vw'
  .split(' ')
  .forEach((unit) => {
    if (!CSS[unit]) {
...
styleIsolationFrame.style.cssText =
...
const USE_CSS_ELEMENT =
  (testStyles.backgroundImage = `-moz-element(#${GLOBAL_ID})`) ===
  testStyles.backgroundImage;
...
function registerPaint(name, Painter, worklet) {
  ...
  painters[name] = {
    worklet,
    Painter,
    properties: Painter.inputProperties
      ? [].slice.call(Painter.inputProperties)
      : [],
    bit: 0,
    instances: [],
  };
...
function getPainterInstance(painter) {
  ...
  return (
    painter.instances[inst] || (painter.instances[inst] = new painter.Painter())
  );
}
...
function update() {
  ...
  for (let i = 0; i < sheets.length; i++) {
    ...
    try {
      context.rules = node.sheet.cssRules;
    } catch (e) {
      continue;
    }
  ...
  if (rules) {
    for (let j = 0; stack.length > 0; j++) {
      ...
      if (rule.cssRules && rule.cssRules.length > 0) {
        context = walkStyles({ cssRules: rule.cssRules }, iterator, context);
        continue;
      }

      // if this is a CSSStyleRule (classic style), iterate over it
      if (rule.type === CSSRule.STYLE_RULE) {
        const r = iterator(rule, context);
        if (r !== undefined) {
          context = r;
        }
      }
      ...
      // Other grouping rules (@media, @supports, etc.)
      ...
      // classic CSSStyleRule => iterate over it
  ...
  walkStyles(sheet, (rule) => {
    ...
    for (let i = 0; i < rule.style.length; i++) {
      ...
      if (HAS_PAINT.test(value)) {
        declarations += `${prop}: ${value}${priority ? ' !' + priority : ''};`;
      }
    }
    if (!declarations) return;
    let block = `${rule.selectorText}{${declarations}}`;
    let parent = rule;
    while ((parent = parent.parentRule)) {
      if (parent.type === LAYER_RULE) {
        block = `@layer ${parent.name}{${block}}`;
      } else {
        const prefix = parent.cssText.match(/^[\s\S]+?\{/)[0];
        block = `${prefix}${block}}`;
      }
    }
    newSheet += block;
...
function processUpdateQueue() {
  ...
  const disable =
    updateQueue.length &&
    updateQueue.some((el) => el && el.$$needsOverrides === true);
...
function getPaintRuleForElement(element) {
  ...
  if (paintRule == null) {
    let index = overrideStyles.insertRule(
      `[data-css-paint="${paintId}"] {}`,
      overrideStyles.cssRules.length,
    );
  ...
  if (element.$$paintObservedProperties && !element.$$needsOverrides) {
    for (let i = 0; i < element.$$paintObservedProperties.length; i++) {
      ...
      if (
        ...
        element.$$paintedPropertyValues[prop]
      ) {
    ...
    if (HAS_PAINT.test(styleAttr)) {
      element.style.cssText =
        styleAttr.replace(/;\s*$/, '') + '; ' + element.style.cssText;
...
const propertiesContainer = {
  ...
  get(name) {
    ...
    let v =
      def && def.inherits === false
        ? currentElement.style.getPropertyValue(name)
        : propertiesContainer.getRaw(name);
  ...
  new window.ResizeObserver((entries) => {
    for (let i = 0; i < entries.length; i++) {
      ...
      if (geom) geom.live = true;
        ...
        geom = entry.target.$$paintGeometry = {
          width: 0,
          height: 0,
          live: true,
        };
      ...
      if (bbox) {
        ...
        geom.width = Math.round(
          (entry.contentRect.right - entry.contentRect.left ||
            entry.contentRect.width) + paddingX,
        );
        geom.height = Math.round(
          (entry.contentRect.bottom - entry.contentRect.top ||
            entry.contentRect.height) + paddingY,
        );
      }
...
function updateElement(element, computedStyle) {
  ...
  let style = (currentProperties =
    computedStyle == null ? getComputedStyle(element) : computedStyle);
  ...
  for (let i = 0; i < style.length; i++) {
    ...
    if (/border-image/.test(property)) {
      ...
      const borderWidth = parseCssDimensions(
        propertiesContainer.getRaw('border-width').split(' '),
      );
      const outset = parseCssDimensions(
        propertiesContainer.getRaw('border-image-outset').split(' '),
      );
      ...
      w += applyDimensions(
        (slice.left != '0' && parseFloat(borderWidth.left)) || 0,
        outset.left || 0,
        true,
      );
      w += applyDimensions(
        (slice.right != '0' && parseFloat(borderWidth.right)) || 0,
        outset.right || 0,
        true,
      );
      h += applyDimensions(
        (slice.top != '0' && parseFloat(borderWidth.top)) || 0,
        outset.top || 0,
        true,
      );
      h += applyDimensions(
        (slice.bottom != '0' && parseFloat(borderWidth.bottom)) || 0,
        outset.bottom || 0,
        true,
      );
    ...
    while ((token = reg.exec(value))) {
      ...
      let equivalentDpr =
        disableScaling || contextOptions.scaling === false ? 1 : dpr;

      let inst;
      if (painter) {
        if (painter.Painter.inputProperties) {
          observedProperties.push.apply(
            observedProperties,
            painter.Painter.inputProperties,
          );
        }
      ...
      const requiresNewBackingContext =
        USE_CSS_CANVAS_CONTEXT === true && ctx && cssContextId !== ctx.id;

      if (
        !canvas ||
        canvas.width != actualWidth ||
        canvas.height != actualHeight ||
        requiresNewBackingContext
      ) {
      ...
      if (inst) {
        ...
        if (
          USE_CSS_CANVAS_CONTEXT === false &&
          !USE_CSS_ELEMENT &&
          'resetTransform' in ctx
        ) {
          ctx.resetTransform();
        }
      }
    ...
    if (
      hasPaints === false &&
      paintedProperties != null &&
      paintedProperties[property] != null
    ) {
  ...
  element.$$paintObservedProperties =
    observedProperties.length === 0 ? null : observedProperties;
  ...
  fetchText(url, (code) => {
    ...
    let parent =
      (styleIsolationFrame.contentDocument &&
        styleIsolationFrame.contentDocument.body) ||
      root;
...
function init() {
  ...
  new MutationObserver((records) => {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (record.type === 'childList') {
        ...
        if (
          record.attributeName === 'data-css-paint' &&
          record.oldValue &&
          target.$$paintId != null &&
          !target.getAttribute('data-css-paint')
        ) {
  ...
  const setAttributeDesc = Object.getOwnPropertyDescriptor(
    Element.prototype,
    'setAttribute',
  );
  ...
  const removeAttributeDesc = Object.getOwnPropertyDescriptor(
    Element.prototype,
    'removeAttribute',
  );
  ...
  let styleDesc = Object.getOwnPropertyDescriptor(
  ...
  let cssTextDesc = Object.getOwnPropertyDescriptor(
  ...
  const properties = Object.keys(
    (window.CSS2Properties || CSSStyleDeclaration).prototype,
  ).filter((m) => IMAGE_CSS_PROPERTIES.test(m));
  properties.forEach((prop) => {
    ...
    propDescs[prop] = {
      ...
      enumerable: true,
      get() {
        const v = this.getPropertyValue(n);
        const pri = this.getPropertyPriority(n);
        return v + (pri ? ' !' + pri : '');
      },
      set(value) {
        const str = String(value);
        if (!HAS_PAINT.test(str)) {
          this.setProperty(n, str);
          return;
        }
        const match = str.match(/^(.*?)\s*(?:!\s*(important)\s*)?$/);
        const val = match?.[1] || '';
        const pri = match?.[2] || '';
        this.setProperty(n, val, pri);
        const owner = this.ownerElement;
        if (owner) {
          ensurePaintId(owner);
          owner.$$needsOverrides = true;
  ...
  let setPropertyDesc = Object.getOwnPropertyDescriptor(
    CSSStyleDeclaration.prototype,
    'setProperty',
  );
  ...
  new MutationObserver((records) => {
    for (const rec of records) {
      if (rec.attributeName === 'class' || rec.attributeName === 'data-theme') {
        processItem('[data-css-paint]', true);
        break;
      }
    }
